...
- **팀 & IP 거버넌스:** 상금 1/N 정산 및 기존 개인 자산(`x1zzLang`)과 대회 신규 산출물 간의 IP 귀속 범위를 명확히 정립.
- **위기 대응:** 팀원 이탈 상황에서 신속히 스코프를 재조정하여 마감 기한 내 완성도 높은 프로젝트 제출 성공.
- **기술 방어 논리:** 대량 쿼리 오버헤드 및 차등 프라이버시(DP) 성능 저하 등 심사위원 예상 질의에 대한 기술적 대응책 수립.
- **보안 아키텍처:** Rust 및 온프레미스 sLM(Qwen2.5-Coder) 기반의 AST 정적 보안 통제 계층 `x1zz Guard` 구축.

#### **GEEKs Hackathon (2026.08.04 - 2026.08.05)**
> **역할:** 팀 발표자, 전략적 피봇 주도, 제품/UX 기획 및 검증
- **전략적 피봇:** 팀 스택 한계를 고려해 복잡한 B2B PaaS에서 실현 가능한 B2C 민원 플랫폼('우선해줘')으로 신속 전환.
- **로직 & UX 기획:** Vision AI(Claude Opus 5)와 PostGIS 공공데이터를 결합한 행정 정당성 스코어링 논리 기획.
- **발표 전담:** 기획 전환 후 발표 준비 및 실전 8분 피칭을 전담하여 프로젝트 완성 및 공유 완료.
---
